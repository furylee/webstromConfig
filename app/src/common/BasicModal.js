...
export default function BasicModal(loadEffect, modal) {
  ...
  const basic = {
    ...
    effects: {
      *loadData(action, { select, put }) {
        ...
        const spaceIds = yield select(state => state[modal.namespace].spaceIds);
        yield put({ type: loadEffect, query, pager, spaceIds });
      },
